...
- **Not driven live.** Handle and usage rejections (exit 2), a missing `cwd`, an argument containing a space, a config backend with no file, and a PATH with no multiplexer. `tests/mux.test.sh` proves those against a scratch repo and a dedicated tmux session.
