...
- **Count before.** `verify-smile panes` prints one object per window. A fresh fixture has exactly one, the shell `up` opened.
...
- **Double kill.** Run `verify-smile kill-pane <bead>` again on the same bead. Exit is still `0`: a pane known to be gone is not an error.
