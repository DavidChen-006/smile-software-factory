...
- The helper points `HOME` at `~/.smile-verify/<runid>/home`, because the driver writes the trust flag into `~/.claude.json` and treehouse pools its worktrees under `$HOME`. It symlinks `Library` into that scratch home, since git's `osxkeychain` credential helper looks for the login keychain there and a fetch from GitHub fails without it, and keeps gh and git pointed at the real config with `GH_CONFIG_DIR` and `GIT_CONFIG_GLOBAL`.
