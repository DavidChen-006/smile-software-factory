...
5. If a pull request for this branch already exists, do not open another: read every open issue labeled `review` whose body names `PR: #<its number>`, fix each finding, commit with `addresses #<issue>` in the message, push, and stop.
6. Stop. Never merge, never close the bead, never run the driver.
