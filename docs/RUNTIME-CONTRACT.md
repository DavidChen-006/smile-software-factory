...
Fix rounds need no special trigger: a new head SHA has no verdict, so the next tick reviews it with `{{issues}}` filled. Workers write `addresses #<i>` in fix commits by convention; the lane does not parse it.
