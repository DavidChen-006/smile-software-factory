...
| `worker.permission_mode` | `bypassPermissions` | Permission mode the worker runs under. `bypassPermissions` since the S9 ruling (2026-09-19): a print-mode worker has nobody to approve a shell call, and the factory has no human in the loop by design; the worker is confined by its own worktree, not by prompts. |
...
`smile narrate` prints the events appended since its last call, one line each, so a planner session that wakes up can relay what happened. It reads only `<factory>/events.jsonl` and `<factory>/narrate.cursor`.

**Cursor.** `<factory>/narrate.cursor` holds one line, `<byte offset>\n`, the count of bytes of the log already narrated. Absent means 0. A file whose content is not `^[0-9]+\n$` is one stderr line, exactly `bad narrate.cursor` with no `smile: ` prefix, and exit 1 with nothing printed and no cursor written. A missing `events.jsonl` is nothing new: nothing printed, exit 0, cursor untouched. An offset beyond the file's current size (the log was recreated) is treated as 0. The cursor is written after printing, to `narrate.cursor.tmp` then renamed over the original.

**Output.** The runtime reads from the offset to the end of the file. Only complete lines (ending in `\n`) count; a trailing partial line is left for the next call. Each complete line that parses as a section 3 event prints as `<ts> <event> <bead> <pr> <actor> <detail>` with null fields as `-` and `detail` verbatim; a line that does not parse, or parses but is not a section 3 object (the same validity check `smile status` uses), prints as `<offset> unreadable` where `<offset>` is its absolute byte offset in the log. Within one call, escalation lines print first, in log order, then everything else in log order: an escalation is `watch.escalation`, `driver.paused`, or `worker.crashed` with `detail` `escalated`. Nothing printed means nothing new: stdout empty, cursor unchanged, exit 0. Exit 0 whenever the read succeeded; extra arguments are exit 2. `smile narrate` appends no event.
...
**cmux.** `spawn` places the pane in a cmux workspace named after the main checkout's directory basename (`smile-verify-<runid>` in the fixture), creating the workspace when absent, with `<cwd>` as the working directory and the argv passed unjoined. `kill` names the workspace explicitly (the `--workspace` argument on close; a close without it acts on the focused workspace, the 2026-07-20 finding). `alive` is 0 while the pane exists and its process has not exited, 1 otherwise, using cmux's own listing, never a screen scrape. Unverified as of 2026-09-19: no cmux server was running when S8 landed and the app was not launched, so the merged argv (`list-workspaces --id-format both`, `new-workspace --name <basename> --cwd <cwd> --focus false`, `new-surface --type terminal --workspace workspace:<n> --focus false`, `send ... "cd <cwd> && exec <argv>"`, `list-pane-surfaces`, `close-surface --surface surface:<m> --workspace workspace:<n>`) is read from `cmux --help` and is not pinned; handle `cmux:<workspace index>:<surface index>`. The unit matrix prints `skipped cmux server not answering` until someone runs it live; the first live run pins or corrects this paragraph.

**Herdr.** Handle `herdr:w<N>:p<N>`, the pane id Herdr returned. `spawn` (pinned 2026-09-19 from PR #47, run live in workspace `w0` and verified in `w13`): pick the workspace (`HERDR_WORKSPACE_ID` when set, else the workspace `herdr workspace list` marks focused, else the first listed); take the first pane of `herdr pane list --workspace <ws>` as the split target (an empty workspace is `herdr refused: no pane in workspace <ws>`, exit 1); `herdr pane split <root> --direction down --cwd <cwd> --no-focus`; `herdr pane rename <pane> <name>`; poll `herdr pane process-info --pane <pane>` until `foreground_processes` is non-empty (5 s cap: a `pane run` typed before the shell is up is silently dropped); then `herdr pane run <pane> "exec <argv shlex-quoted>"`. The argv therefore crosses the pane's shell, quoted so spaces survive; `exec` makes the pane end with the command. `alive`: `herdr pane get <pane>`, 0 on a pane object, 1 on `pane_not_found`. `kill`: `herdr pane close <pane>`, `pane_not_found` swallowed to exit 0; no server answering is exit 1 with one stderr line (section 7; the merged code exits 0 there, a known defect carried to the next builder brief). Herdr must be running (`HERDR_ENV=1` or a reachable `HERDR_SOCKET_PATH`); when the CLI is on PATH but no server answers, `spawn` is one stderr line and exit 1, like a missing tool. A Herdr pane inherits the Herdr server's environment, not the driver's, so nothing the driver exports reaches the worker unless it is in the argv or the worktree; section 7's "inherits the caller's environment" holds for tmux only.

**Prototype first.** The argv above were prototyped on this machine and recorded with observed output in PR #47; `~/.claude/skills/herdr/SKILL.md` documents the Herdr CLI, `cmux --help` the cmux one. A backend paragraph marked unverified is pinned by its first live run, never by reading help text.

**Auto-detect.** Unchanged from section 7: config `backend` names one of the three, else the first of tmux, cmux, herdr on PATH.

**Unit matrix.** `tests/test_mux.py` runs its verb tests once per backend whose tool is on PATH and whose server (Herdr) answers; a backend it cannot run prints one line `skipped <backend> <reason>` and is not a pass. Same handles, same exit codes, same `smile mux alive` semantics across all three; the driver tests keep running on tmux only. The section 8 sequence to `campaign.complete` on Herdr is unproven by `verify-smile` (its stub worker takes GitHub credentials from the fixture tmux session environment, which a Herdr pane never sees); proving it needs the harness to pass the stub shape and token another way, tracked as a harness item.
...
S9 proves the factory on a fresh fixture with real workers and the real reviewer, and writes the two documents a user reads first. Nothing in sections 1 to 14 changes except the one ruling below.

**Ruling, worker argv.** Section 8's default worker argv gains `-p`: `claude -p --model <worker.model> --permission-mode <worker.permission_mode> <order text>`. An interactive `claude` never exits on its own, so the pane never dies and the driver never sees the worker finish; print mode exits when the order is done, which is what `alive` and the waiting state are built on. The `SMILE_WORKER_CMD` path is unchanged. The fix-round respawn of section 9 uses the same argv.

**Ruling, worker permission mode.** Default `worker.permission_mode` is `bypassPermissions` (section 2). Found in the first dogfood run: under `-p` with `acceptEdits` the worker wrote `hello.py`, then its first shell call (running the file, `git`, `gh`) had nobody to approve it and the run ended with no PR, which the driver counts as a crash; two crashes escalated and the watchtower paused the campaign. David's ruling: the whole point is no human in the loop, so the worker runs with permissions bypassed. The reviewer stays in `plan`.

**Ruling, Herdr kill.** `kill` under Herdr exits 1 with one stderr line when no server answers (section 7); only `pane_not_found` is swallowed to 0. This corrects the code merged in #47.

**Harness.** `verify-smile tick` gains `--worker real` and `--reviewer real`. `--worker real` runs the tick with `SMILE_WORKER_CMD` unset, so the driver launches the default argv above, and with the caller's real `HOME` (the scratch HOME is not signed in), while `GH_TOKEN` and `GH_CONFIG_DIR` keep riding the fixture tmux session so the worker's `git push` and `gh pr create` reach the fixture repo and nothing else; the trust edit therefore lands in the real `~/.claude.json` for the fixture worktree paths, which the verifier reports. `--reviewer real` is `review --reviewer real` applied to every review the tick runs. The stub watchtower stays. `verify-smile seed` gains `--title <text>` and `--description <text>`, applied to every bead the call creates, so a dogfood bead carries a real order.

**Campaign.** Fixture config `worker.model: opus` (the default; Opus is the floor for every agent, `docs/PROCESS.md`), `reviewer.models: opus`, `worker.permission_mode: bypassPermissions`. Two beads, seeded one call each: bead 1 "add `hello.py` at the repo root: `uv run hello.py` prints `hello`, `uv run hello.py --name X` prints `hello X`" and bead 2, blocked on bead 1, "add `test_hello.py` covering both outputs of `hello.py`, runnable with `uv run test_hello.py`". The verifier ticks until `campaign.complete`, ticking `--worker real --reviewer real` every time.

**Documents.** `README.md` is the install how-to: what SMILE is in three sentences, prerequisites as `smile doctor` lists them, the four commands to go from a clean repo to a running campaign (`uv run install.py <repo>`, `smile init`, `bd create`, `smile run`), how to watch (`smile narrate`, the panes, the watchtower report), how to stop (`smile pause`, `smile resume`), and a pointer to `docs/HOW-IT-WORKS.md`. `docs/HOW-IT-WORKS.md` is the explanation: the four lanes, the event log as the only shared truth, the tick in four steps, the review lane and the fix round, the watchtower and narration, the mux seam and its three backends, and the invariant `smile audit` guards. Neither document names a model, a price, a person, or a path under a home directory; both are checked by the forbidden-word grep of section 13 extended to `README.md` and `docs/HOW-IT-WORKS.md`, and `tests/skills.test.sh` runs that grep.
