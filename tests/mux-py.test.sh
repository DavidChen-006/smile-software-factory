...
(cd "$ROOT" && python3 -m unittest tests.test_mux -v) || { printf 'FAIL mux-py.test.sh\n'; exit 1; }

if [ -x "$FLOOR" ] && "$FLOOR" -c 'import sys; sys.exit(sys.version_info[:2] != (3, 9))'; then
	BIN=$(mktemp -d)
	trap 'rm -r -- "$BIN"' EXIT
	ln -s "$FLOOR" "$BIN/python3"
	(cd "$ROOT" && PATH="$BIN:$PATH" "$FLOOR" -m unittest tests.test_mux -v) ||
		{ printf 'FAIL mux-py.test.sh (python3.9 pass)\n'; exit 1; }
else
